...
export class ChatDB {
    ...
    this.dbPromise = new Promise((resolve, reject) => {
      ...
      req.onupgradeneeded = () => {
        ...
        if (!db.objectStoreNames.contains("messages")) {
          ...
          m.createIndex("by_session_createdAt", ["sessionId", "createdAt"], {
            unique: false,
          });
  ...
  async updateMessage(
    id: string,
    patch: Partial<Pick<ChatMessage, "content" | "sources">>,
  ): Promise<void> {
    let sessionId: string | null = null;
    await this.tx(["messages"], "readwrite", async (tx) => {
      const store = tx.objectStore("messages");
      const m =
        (await reqToPromise<ChatMessage | undefined>(store.get(id))) ?? null;
      if (!m) return;
